...
const PRIMARY_GOALS = [
  ...
  },
  {
    ...
    desc: "Stay at my current weight and tone up",
  },
  {
    value: "manage_stress",
    title: "Manage Stress",
    desc: "Use exercise for mental well-being and relaxation",
  },
  {
    value: "improve_flexibility",
    title: "Improve Flexibility",
    desc: "Enhance mobility, range of motion, and posture",
  },
  {
    value: "build_endurance",
    title: "Build Endurance",
    desc: "Increase stamina for runs, hikes, or sports",
  },
  {
    value: "increase_energy",
    title: "Increase Energy",
    desc: "Boost daily energy levels and overcome fatigue",
  },
];
...
const ACTIVITY_LEVELS = [
  {
    ...
    desc: "Mostly sitting (office work, driving, etc.)",
  },
  {
    value: "lightly_active",
    title: "Lightly Active",
    desc: "On your feet often (teaching, retail, light walks)",
  },
  {
    value: "active",
    title: "Active",
    desc: "Moving most of the day or regular exercise",
  },
  {
    value: "very_active",
    title: "Very Active",
    desc: "Heavy physical labor or intense daily training",
  },
];
...
const FITNESS_LEVELS = [
  ...
  },
  {
    value: "intermediate",
    title: "Intermediate",
    desc: "I exercise 1-3 times a week consistently.",
  },
  {
    value: "advanced",
    title: "Advanced",
    desc: "I train 4+ times a week and know my way around a gym.",
  },
];
...
function OnboardingQuestions() {
  ...
  const isDetailsValid = useCallback(() => {
    if (!formData.gender || !formData.dob || !formData.location.trim())
      return false;
    return validateAge(formData.dob).valid;
  }, [formData.gender, formData.dob, formData.location]);
  ...
  const validateMetrics = useCallback(() => {
    ...
    const isGoalEmpty =
      !formData.goalWeightValue ||
      String(formData.goalWeightValue).trim() === "";
    ...
    if (isHeightValid && isWeightValid && isGoalWeightValid) {
      if (!isGoalEmpty) {
        ...
        if (isMuscleGain && goalWeight <= currentWeight) {
          return {
            valid: false,
            error: "Goal weight must be greater than your current weight.",
          };
        }
      }
    ...
    if (
      ...
      return {
        valid: false,
        error: "Please enter realistic height and weight values.",
      };
    }
  ...
  }, [currentStep, totalSteps, updateUserData, formData, navigate]);
    ...
    const handleKeyDown = (e) => {
      if (e.key === "Enter") {
        ...
        if (currentStep === 0) canProceed = true;
          ...
          canProceed = formData.primaryGoal.length > 0;
  ...
  const handleDobChange = (value) => {
    ...
    if (underage) {
      setAgeError(
        "Sorry, you must be at least 18 years old. Redirecting to home...",
      );
      if (ageTimerRef.current) clearTimeout(ageTimerRef.current);
  ...
  const handleToggleHeight = (e) => {
    ...
    setFormData((prev) => {
      if (prev.heightUnit === "imperial") {
        const totalInches =
          (parseFloat(prev.heightFeet) || 0) * 12 +
          (parseFloat(prev.heightInches) || 0);
        return {
          ...prev,
          heightUnit: "metric",
          heightCm: totalInches > 0 ? (totalInches * 2.54).toFixed(1) : "",
        };
      } else {
    ...
    setFormData((prev) => {
      if (prev.weightUnit === "imperial") {
        ...
        return {
          ...
          weightValue:
            !isNaN(lbs) && lbs > 0
              ...
              : prev.weightValue,
          goalWeightValue:
            !isNaN(goalLbs) && goalLbs > 0
              ? (goalLbs * 0.453592).toFixed(1)
              : prev.goalWeightValue,
        };
      } else {
        ...
        return {
          ...
          weightValue:
            !isNaN(kgs) && kgs > 0
              ? (kgs / 0.453592).toFixed(1)
              : prev.weightValue,
          goalWeightValue:
            !isNaN(goalKgs) && goalKgs > 0
              ? (goalKgs / 0.453592).toFixed(1)
              : prev.goalWeightValue,
        };
      }
    });
      ...
      <main className={styles.onboardingContainer}>
        ...
        {currentStep === 0 && (
          <div className={styles.step}>
            <div className={styles.welcomeHeader}>
              <h2>
                Welcome to{" "}
                <span className={styles.gradientText}>Health & Fitness</span>
              </h2>
              <p className={styles.subtitle}>
                Everything you need in one place — track what you eat, follow a
                workout plan, and watch your body actually change.
              </p>
              <p
                className={styles.subtitle}
                style={{ fontSize: "0.9rem", marginTop: "12px" }}
              >
                Let's take 60 seconds to personalize your experience.
              </p>
            </div>
            <div className={styles.imagesContainer}>
              <div className={styles.imageWrapper}>
                <img src={workoutImg} alt="Workout Training" />
              </div>
              <div className={styles.imageWrapper}>
                <img src={mealImg} alt="Healthy Nutrition" />
              </div>
              <div className={styles.imageWrapper}>
                <img src={situpsImg} alt="Core Training" />
              </div>
            </div>
        ...
        {currentStep === 2 && (
          <div className={styles.step}>
            <div className={styles.questionHeader}>
              <h2>{goalGreeting}</h2>
              <p className={styles.subtitle}>
                Select up to two to help us tailor your experience.
              </p>
            </div>
            <div className={styles.optionsList}>
              {PRIMARY_GOALS.map((option) => (
                <div
                  ...
                  onClick={() =>
                    handleCardSelect("primaryGoal", option.value, 2)
                  }
                >
                  <h3>{option.title}</h3>
                  <p>{option.desc}</p>
            ...
            <div className={styles.navigationButtons}>
              ...
              <button
                ...
                className={styles.ctaBtn}
                onClick={handleNext}
                disabled={formData.primaryGoal.length === 0}
              >
                Next
              </button>
            </div>
            ...
            <div className={styles.optionsList}>
              {ACTIVITY_LEVELS.map((option) => (
                <div
                  ...
                  onClick={() =>
                    handleCardSelect("activityLevel", option.value, 1)
                  }
                >
                  <h3>{option.title}</h3>
                  <p>{option.desc}</p>
        ...
        {currentStep === 4 && (
          <div className={styles.step}>
            <div className={styles.questionHeader}>
              ...
              <p className={styles.subtitle}>
                Be honest, this sets your starting point.
              </p>
            </div>
            <div className={styles.optionsList}>
              {FITNESS_LEVELS.map((option) => (
                <div
                  ...
                  onClick={() =>
                    handleCardSelect("fitnessLevel", option.value, 1)
                  }
                >
                  <h3>{option.title}</h3>
                  <p>{option.desc}</p>
        ...
        {currentStep === 5 && (
          <div className={styles.step}>
            ...
            <div className={styles.formGroup}>
              {ageError && (
                <div className={styles.errorMessage}>{ageError}</div>
              )}
                    ...
                    <div className={styles.inputGroupVertical}>
                      ...
                      >
                        <input
                          ...
                          className={styles.textInput}
                          placeholder={
                            formData.weightUnit === "imperial" ? "lbs" : "kg"
                          }
                          min="0"
                          step="0.1"
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({
                              ...prev,
                              goalWeightValue: e.target.value,
                            }))
                          }
                        />
                        <span className={styles.unitLabel}>
