        ...
        <nav className={styles.sidebarNav}>
          {navItems.map((item) => (
            <Link
              key={item.id}
              to={item.path}
              className={`${styles.navItem} ${location.pathname.startsWith(item.path) ? styles.active : ""} ${isCollapsed ? "sidebarCollapsedTooltip" : ""}`}
              onClick={onClose}
          ...
          <button 
            className={`${styles.navItem} ${styles.quickActionBtn} ${isCollapsed ? "sidebarCollapsedTooltip" : ""}`} 
            onClick={() => { toggleQuickLog(true); onClose(); }}
            style={{ marginTop: 'auto' }}
            data-tooltip="New Entry (Ctrl+K)"
          >
