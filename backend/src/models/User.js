import mongoose from 'mongoose';
import bcrypt from 'bcryptjs';

const userSchema = mongoose.Schema({
    name: { type: String, required: true },
    email: { type: String, required: true, unique: true },
    password: { type: String, required: true },
    firstName: { type: String },
    lastName: { type: String },
    dob: { type: Date },
    age: { type: Number },
    gender: { type: String },
    location: { type: String },
    primaryGoal: [{ type: String }],
    fitnessLevel: { type: String },
    activityLevel: { type: String },
    heightUnit: { type: String, default: 'imperial', enum: ['metric', 'imperial'] },
    heightFeet: { type: String },
    heightInches: { type: String },
    heightCm: { type: String },
    height: { type: Number }, // legacy support
    weightUnit: { type: String, default: 'imperial', enum: ['metric', 'imperial'] },
    weightValue: { type: Number },
    goalWeightValue: { type: Number },
    emailNotifications: { type: Boolean, default: true },
    smsReminders: { type: Boolean, default: false },
    publicProfile: { type: Boolean, default: true },
    role: { type: String, default: 'user' },
    lastLoginDate: { type: String }, // YYYY-MM-DD format
    loginStreak: { type: Number, default: 1 }
}, { timestamps: true });

// Match user entered password to hashed password in database
userSchema.methods.matchPassword = async function(enteredPassword) {
    return await bcrypt.compare(enteredPassword, this.password);
};

// Encrypt password using bcrypt
userSchema.pre('save', async function() {
    if (!this.isModified('password')) {
        return;
    }

    const salt = await bcrypt.genSalt(10);
    this.password = await bcrypt.hash(this.password, salt);
});

const User = mongoose.model('User', userSchema);

export default User;
