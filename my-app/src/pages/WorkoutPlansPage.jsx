...
function WorkoutPlansPage() {
    ...
    const [exercises, setExercises] = useState(() => {
        return JSON.parse(localStorage.getItem('loggedExercises_grouped')) || defaultExercises;
    });
    ...
    const completionPercent = useMemo(() => {
        ...
        return Math.round((exercises.filter(e => e.completed).length / exercises.length) * 100);
    }, [exercises]);
    ...
    return (
        <div className={dashStyles.pageWrapper}>
                ...
                <main className={pageStyles.dashboardContent}>
                    ...
                    <section className={pageStyles.logSection}>
                        <div className={pageStyles.logHeader}>
                            ...
                            <button className={pageStyles.primaryBtn} onClick={() => setIsModalActive(true)}>Add Exercise</button>
                        </div>
                        <div className={pageStyles.muscleGroups}>
                            {Object.keys(mainGroups).length > 0 ? (
                                ...
                                <div className={pageStyles.emptyState}>No pending main exercises.</div>
                            )}
                        </div>
                    </section>
                    ...
                    <section className={pageStyles.logSection}>
                        <div className={pageStyles.sectionHeader}>
                            ...
                            <p className={pageStyles.sectionGoal}>Goal: 5-10 mins static stretching</p>
                        </div>
                        <div className={pageStyles.exerciseList}>
                    ...
                    <section className={pageStyles.workoutSummary}>
                        <div className={pageStyles.summaryCard}>
                            ...
                            <span className={pageStyles.value}>{workoutSummary.total} Tasks</span>
                        </div>
                        <div className={`${pageStyles.summaryCard} ${pageStyles.highlight}`}>
                            <span className={pageStyles.label}>Completed</span>
                            <span className={pageStyles.value}>{workoutSummary.completed} Done</span>
                        </div>
                        <div className={pageStyles.summaryCard}>
                            <span className={pageStyles.label}>Energy Burned</span>
                            <span className={pageStyles.value} style={{ color: '#f59e0b' }}>{workoutSummary.totalCals} <small>kcal</small></span>
                        </div>
                    </section>
                    ...
                    <section className={pageStyles.workoutProgress}>
                        ...
                        <div className={pageStyles.progressBar}>
                            <div className={pageStyles.progressFill} style={{ width: `${completionPercent}%` }}></div>
                        </div>
                    </section>
                </main>
            </div>
            ...
            {/* Modal */}
            <div className={`${pageStyles.modalOverlay} ${isModalActive ? pageStyles.active : ''}`}>
                <div className={pageStyles.modalContent}>
                    <h3>Log New Exercise</h3>
                    <form onSubmit={handleFormSubmit}>
                        <div className={pageStyles.formGroup}>
                            <label>Exercise Name</label>
                            <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required placeholder="e.g. Pushups" />
                        </div>
                        <div className={pageStyles.formGroup}>
                            <label>Category</label>
                            <select className={pageStyles.customSelect} value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                        ...
                        {formData.category === 'main' && (
                            <div className={pageStyles.formGroup}>
                                ...
                                <input type="text" value={formData.muscleGroup} onChange={e => setFormData({...formData, muscleGroup: e.target.value})} required placeholder="e.g. Chest, Legs" />
                            </div>
                        )}
                        <div className={pageStyles.formRow}>
                            <div className={pageStyles.formGroup}>
                                <label>Sets</label>
                                <input type="number" value={formData.sets} onChange={e => setFormData({...formData, sets: e.target.value})} required placeholder="0" />
                            </div>
                            <div className={pageStyles.formGroup}>
                                <label>Reps / Duration</label>
                                <input type="text" value={formData.reps} onChange={e => setFormData({...formData, reps: e.target.value})} required placeholder="e.g. 10 or 30s" />
                            </div>
                            <div className={pageStyles.formGroup}>
                                <label>Calories</label>
                                <input type="number" value={formData.calories} onChange={e => setFormData({...formData, calories: e.target.value})} placeholder="e.g. 50" />
                            </div>
                        </div>
                        <div className={pageStyles.modalActions}>
                            <button type="button" className={pageStyles.btnCancel} onClick={() => setIsModalActive(false)}>Cancel</button>
                            <button type="submit" className={pageStyles.primaryBtn}>Add to Log</button>
                        </div>
                    </form>
                </div>
            </div>
