...
const nutrientData = [
    {
        ...
        title: 'Protein',
        type: 'Macronutrient',
        brief: 'Large molecules consisting of amino acids that our bodies need to function properly.',
        importance: 'Essential for building and repairing tissues, preserving muscle mass, and making enzymes and hormones.',
    ...
    },
    {
        ...
        title: 'Carbohydrates',
        type: 'Macronutrient',
        brief: 'Sugar molecules that your body breaks down into glucose for energy.',
        importance: "Your body's primary and preferred source of energy. Fuels brain function and physical activity.",
    ...
    },
    {
        ...
        importance: 'Crucial for nutrient absorption, nerve transmission, protecting organs, and hormone production.',
    ...
    },
    {
        ...
        type: 'Micronutrient',
        brief: 'Essential dietary elements needed in varying, but small quantities.',
        importance: 'Orchestrate bone health, brain function, immune system boosting, and disease prevention.',
    ...
    },
    {
        ...
        brief: 'The most crucial nutrient; you can only survive a few days without it.',
        importance: 'Acts as a solvent and temperature regulator. Flushes waste, lubricates joints, aids in digestion and nutrient absorption.',
...
const WaterIcon = () => (
    <svg viewBox="0 0 24 24" fill="none" stroke="var(--accent-cyan)" strokeWidth="2" width="24" height="24">
...
function NutritionPage() {
    ...
    // Accordion state — tracks which nutrient IDs are expanded
                ...
                <main className={styles.nutritionDashboard}>
                    ...
                    <section className={styles.understandingNutrientsSection}>
                        ...
                        <p className={styles.sectionIntro}>
                            Nutrients are chemical compounds in food that are crucial to human growth, disease prevention, and overall health. They are divided into macros (needed in large amounts) and micros (needed in smaller amounts).
                        </p>
                            ...
                            {nutrientData.map(nutrient => {
                                ...
                                return (
                                    <div
                                        ...
                                        onKeyDown={e => e.key === 'Enter' && toggleNutrient(nutrient.id)}
                                    >
                                        ...
                                        {isExpanded && (
                                            <div className={styles.nutrientDropdownContent}>
                                                <div className={styles.nutrientDropdownInner}>
                                                    <p><strong>Importance & Function:</strong> {nutrient.importance}</p>
                                                    <p><strong>Food Sources:</strong> {nutrient.sources}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className={styles.dashboardGrid}>
                        <aside className={styles.sidebarColumn}>
                            ...
                            {/* Calorie Calculator */}
                            <section className={`${styles.card} ${styles.interactiveCard} ${styles.calculatorSection}`}>
                                <h2>Calculate Target</h2>
                                <p className={styles.calcDesc}>Enter your details to generate personalized targets.</p>

                                <form className={styles.calcForm} onSubmit={handleCalcSubmit}>

                                    {/* Unit Toggle */}
                                    <div className={styles.unitToggle}>
                                        <input type="radio" id="unit-metric" name="unit-type" value="metric"
                                            checked={unit === 'metric'}
                                            onChange={() => setUnit('metric')} />
                                        <label htmlFor="unit-metric" className={styles.toggleBtn}>Metric</label>

                                        <input type="radio" id="unit-imperial" name="unit-type" value="imperial"
                                            checked={unit === 'imperial'}
                                            onChange={() => setUnit('imperial')} />
                                        <label htmlFor="unit-imperial" className={styles.toggleBtn}>Imperial</label>
                                    </div>

                                    <div className={styles.formGroup}>
                                        <label>{unit === 'metric' ? 'Weight (kg)' : 'Weight (lbs)'}</label>
                                        <input type="number" placeholder="e.g. 75" step="0.1"
                                            value={calcWeight}
                                            onChange={e => setCalcWeight(e.target.value)} required />
                                    </div>

                                    <div className={styles.formGroup}>
                                        <label>Primary Goal</label>
                                        <div className={styles.selectWrapper}>
                                            <select value={calcGoal} onChange={e => setCalcGoal(e.target.value)} required>
                                    ...
                                    <div className={styles.formGroup}>
                                        <label>{unit === 'metric' ? 'Height (cm)' : 'Height'}</label>
                                        {unit === 'metric' ? (
                                            <input type="number" placeholder="e.g. 175"
                                                value={calcHeightCm}
                                                onChange={e => setCalcHeightCm(e.target.value)} required />
                                        ) : (
                                    ...
                                    <button type="submit" className={styles.btnPrimary}>Calculate</button>
                                </form>

                                {calcResult !== null && (
                                    <div className={styles.calcResult}>
                                        <h4>Suggested Target</h4>
                                        <div className={styles.resultValue}>{calcResult.toLocaleString()} kcal</div>
                                        <button
                            ...
                            <section className={`${styles.card} ${styles.hydrationSection}`}>
                                ...
                                <div className={styles.hydrationTracker}>
                                    ...
                                    <p className={styles.hydrationStatus}>
                                        {filledGlasses} / {TOTAL_GLASSES} glasses ({filledGlasses * 250}ml)
                                    </p>
                                </div>
                            </section>
